...
function Dashboard() {
  ...
  const handleUpdatePost = async (e) => {
    ...
    setPosting(true)
    try {
      const updatedPost = await postsAPI.updatePost(editingPost._id, {
        content: newPost,
        images: postImages
      })
          ...
          <div className="create-post">
            <form onSubmit={editingPost ? handleUpdatePost : handleCreatePost}>
              ...
              <div className="post-actions-bar">
                ...
                <span className="image-count">{postImages.length}/4 photos</span>
                <button type="submit" className="post-btn" disabled={posting || !newPost.trim()}>
                  {posting ? 'Posting...' : editingPost ? 'Update' : 'Post'}
                </button>
              </div>
            </form>
          </div>
